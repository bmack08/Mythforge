...
class StatBlock extends Model {
	...
	validateStatBlock() {
		...
		if (!this.alignment) errors.push('Alignment is required');
		if (!this.armorClass) errors.push('Armor Class is required');
		if (!this.hitPoints) errors.push('Hit Points is required');
		if (!this.speed) errors.push('Speed is required');
		...
		if (this.attacks && this.attacks.length > 0) {
			this.attacks.forEach((attack, index) => {
				if (!attack.name) errors.push(`Attack ${index + 1} must have a name`);
