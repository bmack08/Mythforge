...
class NPC extends Model {
	...
	validateNPC() {
		...
		if (!this.occupation) errors.push('Occupation is required');
		
		// Validate personality structure
		if (this.personality) {
			if (!this.personality.trait) warnings.push('Personality trait not defined');
			if (!this.personality.ideal) warnings.push('Ideal not defined');
			if (!this.personality.bond) warnings.push('Bond not defined');
			if (!this.personality.flaw) warnings.push('Flaw not defined');
		} else {
		...
		if (this.voice) {
			if (!this.voice.accent) warnings.push('Voice accent not defined');
			if (!this.voice.speechPattern) warnings.push('Speech pattern not defined');
		} else {
	...
	toHomebreweryMarkdown() {
		...
		if (this.stats && this.combatCapable) {
			...
			const statStrings = statOrder.map(stat => 
				`${stat.toUpperCase()} ${this.stats[stat] || 10}`
			);
