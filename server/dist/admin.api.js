...
router.post('/api/lock/:id', mw.adminOnly, asyncHandler(async (req, res) => {
    ...
    if (!brew)
        throw { name: 'Brew Not Found', message: 'Cannot find brew to lock', shareId: req.params.id, status: 500, HBErrorCode: '63' };
...
router.put('/api/unlock/:id', mw.adminOnly, asyncHandler(async (req, res) => {
    ...
    const brew = await HomebrewModel.findOne(filter);
    if (!brew)
        throw { name: 'Brew Not Found', message: 'Cannot find brew to unlock', shareId: req.params.id, status: 500, HBErrorCode: '66' };
    if (!brew.lock)
        throw { name: 'Not Locked', message: 'Cannot unlock as brew is not locked', shareId: req.params.id, status: 500, HBErrorCode: '67' };
    brew.lock = undefined;
    brew.markModified('lock');
    await brew.save()
        .catch((error) => {
...
router.put('/api/lock/review/request/:id', asyncHandler(async (req, res) => {
    ...
    const brew = await HomebrewModel.findOne(filter);
    if (!brew) {
        throw { name: 'Brew Not Found', message: `Cannot find a locked brew with ID ${req.params.id}`, code: 500, HBErrorCode: '70' };
    }
    ;
...
router.put('/api/lock/review/remove/:id', mw.adminOnly, asyncHandler(async (req, res) => {
    ...
    const brew = await HomebrewModel.findOne(filter);
    if (!brew) {
        throw { name: 'Can Not Clear Review Request', message: `Brew ID ${req.params.id} does not have a review pending!`, HBErrorCode: '73' };
    }
    ;
