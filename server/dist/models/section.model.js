...
class Section extends Model {
    ...
    validateStatBlock() {
        ...
        if (!data.name)
            errors.push('Stat block must have a name');
        if (!data.armorClass)
            errors.push('Stat block must have AC');
        if (!data.hitPoints)
            errors.push('Stat block must have HP');
        if (!data.challengeRating)
            errors.push('Stat block must have CR');
        return {
    ...
    validateMagicItem() {
        ...
        const data = this.contentData;
        if (!data.name)
            errors.push('Magic item must have a name');
        if (!data.rarity)
            errors.push('Magic item must have rarity');
        if (!data.description)
            errors.push('Magic item must have description');
    ...
    validateNPC() {
        ...
        const data = this.contentData;
        if (!data.name)
            errors.push('NPC must have a name');
        if (!data.race)
            errors.push('NPC must have a race');
        if (!data.occupation)
            errors.push('NPC must have an occupation');
        return {
            valid: errors.length === 0,
            errors
        };
    ...
    validateLocation() {
        ...
        if (!data.description)
            errors.push('Location must have description');
        return {
            valid: errors.length === 0,
            errors
        };
