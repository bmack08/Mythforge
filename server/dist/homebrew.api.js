...
import { splitTextStyleAndMetadata, brewSnippetsToJSON, debugTextMismatch } from '../shared/helpers.js';
...
const api = {
    ...
    getUsersBrewThemes: async (username) => {
        ...
        const brews = await Homebrew.getByUser(username, true, fields, {
            tags: {
                [Op.or]: [
                    { [Op.like]: '%meta:theme%' },
                    { [Op.like]: '%meta:Theme%' }
    ...
    getBrew: (accessType, stubOnly = false) => {
        ...
        return async (req, res, next) => {
            ...
            const { Homebrew } = getModels();
            let stub = await Homebrew.get(accessMap[accessType])
                .catch((err) => {
                if (googleId)
            ...
            stub = stub?.toJSON ? stub.toJSON() : stub;
    ...
    newBrew: async (req, res) => {
        ...
        let googleId, saved;
        if (saveToGoogle) {
            googleId = await api.newGoogleBrew(req.account, homebrewData, res);
            if (!googleId)
                return;
            api.excludeStubProps(homebrewData);
            homebrewData.googleId = googleId;
        }
        else {
            // Compress brew text to binary before saving
            homebrewData.textBin = zlib.deflateRawSync(homebrewData.text);
            // Delete the non-binary text field since it's not needed anymore
            homebrewData.text = undefined;
        }
        saved = await Homebrew.create(homebrewData)
            .catch((err) => {
        ...
        while (req.params.id) {
            ...
            if (!isStaticTheme(req.params.renderer, req.params.id)) {
                ...
                if (currentTheme?.snippets)
                    completeSnippets.push({ name: currentTheme.title, snippets: currentTheme.snippets });
                if (currentTheme?.style)
                    completeStyles.push(`/* From Brew: ${req.protocol}://${req.get('host')}/share/${req.params.id} */\n\n${currentTheme.style}`);
    ...
    updateBrew: async (req, res) => {
        ...
        let saved;
        if (!brew.id) {
            // if the brew does not have a stub id, create and save it
            saved = await Homebrew.create(brew).catch(saveError);
        }
        else {
            // if the brew does have a stub id, update it using the stub id as the key.
            const existingBrew = await Homebrew.findByPk(brew.id);
            if (existingBrew) {
                Object.assign(existingBrew, brew);
                saved = await existingBrew.save().catch(saveError);
            }
            else {
                saved = await Homebrew.create(brew).catch(saveError);
            }
    ...
    deleteBrew: async (req, res, next) => {
        ...
        try {
            ...
            if (err.HBErrorCode == '02') {
                ...
                const { Homebrew } = getModels();
                await Homebrew.destroy({ where: { editId: id } });
                return next();
            }
        }
        let brew = req.brew;
        ...
        if (brew.id) {
            ...
            if (existingBrew) {
                ...
                if (account) {
                    ...
                    existingBrew.authors = _.pull(existingBrew.authors, account.username);
                }
                if (existingBrew.authors.length === 0) {
                    // Delete brew if there are no authors left
                    await existingBrew.destroy()
                        .catch((err) => {
                        console.error(err);
                        throw { name: 'BrewDelete Error', message: 'Error while removing', status: 500, HBErrorCode: '07', brewId: existingBrew.id };
                    });
                }
                else {
                    if (shouldDeleteGoogleBrew) {
                        ...
                        existingBrew.googleId = undefined;
                        existingBrew.textBin = zlib.deflateRawSync(existingBrew.text);
                        existingBrew.text = undefined;
                    }
                    await existingBrew.save()
                        .catch((err) => {
                        throw { name: 'BrewAuthorDelete Error', message: err, status: 500, HBErrorCode: '08', brewId: existingBrew.id };
                    });
                }
            }
        }
...
export default api;
//# sourceMappingURL=homebrew.api.js.map