...
// Test wizard route
app.get('/test-wizard.html', (req, res) => {
	res.sendFile(`${process.cwd()}/test-wizard.html`);
});
...
const defaultMetaTags = {
	site_name   : 'Mythwright - AI-Powered D&D Sourcebook Generator',
	title       : 'Mythwright',
	description : 'Generate complete D&D sourcebooks with AI-powered content creation, built on Homebrewery technology.',
...
app.get('/user/:username', async (req, res, next)=>{
	...
	let brews = await Homebrew.getByUser(req.params.username, ownAccount, fields)
	.catch((err)=>{
		console.log(err);
	});
...
app.get('/projects', asyncHandler(async(req, res, next)=>{
	req.ogMeta = { ...defaultMetaTags,
		title       : 'My Projects',
		description : 'Manage your AI-generated campaigns and adventures'
	};
...
app.get('/share/:id', asyncHandler(getBrew('share')), asyncHandler(async (req, res, next)=>{
	...
	if(!brew.authors.includes(req.account?.username)){
		if(req.params.id.length > 12 && !brew._id) {
			...
			const { Homebrew } = getModels();
			await Homebrew.increaseView({ shareId: brew.shareId });
		}
	};
...
app.get('/account', asyncHandler(async (req, res, next)=>{
	...
	if(req.account) {
		...
		const mongoCount = await Homebrew.count({ where: query })
			.catch((err)=>{
				mongoCount = 0;
				console.log(err);
			});
